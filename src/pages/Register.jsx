...
import {Alert, Button, DatePicker, Form, Input, Select, SelectItem} from "@heroui/react";
...
const navigate = useNavigate()
  ...
  const formattedData = {
    ...data,
    dateOfBirth: `${data.dateOfBirth.getDate()}-${data.dateOfBirth.getMonth() + 1}-${data.dateOfBirth.getFullYear()}`
  };
  ...
  return (
    ...
    <form className="w-full max-w-4xl space-y-10" onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="input-form space-y-5">  
          ...
          <Input isInvalid={Boolean(errors.name )} errorMessage={errors.name?.message} {...register("name")} isRequired variant="faded" label="Name" placeholder="Enter your name" type="text" />
          ...
          <Input isInvalid={Boolean(errors.rePassword)} errorMessage={errors.rePassword?.message} {...register("rePassword")}  isRequired variant="faded" label="rePassword" placeholder="confirm your password" type="password" />

          <div className="flex gap-5 items-center ">

            <Input isInvalid={Boolean(errors.dateOfBirth)} errorMessage={errors.dateOfBirth?.message}  type="date" {...register("dateOfBirth")} isRequired className="max-w-[284px]" label="Birth date" />
...
>
  {genders.map((gender) => (
          <SelectItem value={`${gender.key}`} key={gender.key}>{gender.label}</SelectItem>
        ))}
</Select>
